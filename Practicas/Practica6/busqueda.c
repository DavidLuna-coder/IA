...
LISTA expandir(tNodo *nodo, int *nSucesores)
{
    unsigned op;
    ...
    for (op = 1; op <= NUM_OPERADORES; op++)
    {
        if (esValido(op, nodo->estado))
        {
            ...
            // nuevo->valHeuristica = heuristicaManhattan(nuevo->estado);
            nuevo->valHeuristica = heuristicaPiezasMal(nuevo->estado);
            (*nSucesores)++;
...
int busquedaProfundidad()
{
    ...
    while (!esVacia(Abiertos) && !objetivo)
    {
        ...
        {
            int temp = 0;
            Sucesores = expandir(Actual, &temp);
...
int busquedaVoraz()
{
    ...
    while (!esVacia(Abiertos) && !objetivo)
    {
        ...
        {
            int nSucesores = 0;
            Sucesores = expandir(Actual, &nSucesores);

            Abiertos = ordenarLista(Abiertos, Sucesores, 0);
            longitudAbiertos += nSucesores;
            generados += nSucesores;
            if (longitudAbiertos > maxAbiertos)
                maxAbiertos = longitudAbiertos;
